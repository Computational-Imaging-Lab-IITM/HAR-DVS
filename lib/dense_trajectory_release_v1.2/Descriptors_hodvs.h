...
//Mat hist_vals;  //create the Mat objects to save our histograms
cv::Mat hist_vals;
/// Compute the histograms:
const cv::Mat mat_input = cv::Mat(1, nDims, CV_32F, const_cast<float *>(xc)).clone();

//calcHist( mat_input, 1, &intensity, Mat(), hist_vals, 1, &histSize, &histRange, uniform, accumulate );  // 8 bins
calcHist( &mat_input, 1, 0, Mat(), hist_vals, 1,  &histSize, &histRange, true, false ); 
		...
		for(int j = 0; j < xComp.cols; j++) {
			...
			for(int m = 0; m < nDims; m++, index++)
				desc[index] = hist_vals.at<float>(m);
		//}
	}
}
...
void HogComp(const Mat& img, float* desc, DescInfo& descInfo)
{
	...
	BuildDescMat(img, img, desc, descInfo);
}
